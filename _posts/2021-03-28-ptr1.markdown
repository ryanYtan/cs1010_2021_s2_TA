...
* Reads, from the standard input, a string $$s$$ that can contain
  ...
  * Whitespace
  * Punctuation from the following set: [`!` `"` `#` `$` `%` `&` `'` `(` `)` `*` `+` `,` `-` `.` `/` `:` `;` `<` `=` `>` `?` `@` `[` `\` `]` `^` `_` `{` `|` `}` `~`]
* Outputs, to the standard output, each word that appears in the reduced input, each on a separate line

It is guaranteed that the length of each word in the input before tokenisation is at most 32 characters long.
