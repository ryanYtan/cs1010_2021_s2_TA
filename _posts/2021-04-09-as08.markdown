...
As you can tell, it doesn't scale up very well. Why is it so slow?
The time complexity of `RecursiveWalk` is given by the recurrence:
...
number of permutations of a collection of 3 $$R$$s and 4 $$L$$s.

$$
Permute(RRRLLLL) = \frac{(N_R + N_L)!}{N_R!N_L!} = \frac{(N_R + N_L)!}{N_R!((N_R + N_L) - N_R)!} = \binom{N_R + N_L}{N_R}
$$

Where $$N_L$$ and $$N_R$$ denote the number of $$L$$ and $$R$$ in the collection
respectively.  Recall that the formula for the binomial coefficient
