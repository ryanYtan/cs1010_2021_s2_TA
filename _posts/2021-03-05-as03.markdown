...
$$Count[0...k]$$ be an array that counts the *frequency* of each
integer that can occur in $$A$$. For any integer
$$m$$ that occurs $$p$$ times in $$A$$, $$Count[m] = p$$.

After we build $$Count$$, all we have to do is run through it and print out
...
### Why is the second loop $$O(n+k)$$ and not $$O(nk)$$?
The second loop _looks_ like we are looping over an array of $$k$$
size, and in the worst case, we are printing out $$n$$ values for each
iteration.

However, is that really true? Note that the value of each $$Count[i]$$ is the
number of times $$i$$ occurs in the original array $$A$$. So, the sum of all
values in $$Count$$ should equal $$n$$. If $$Count[i] = n$$ for every $$i$$,
then the number of elements counted by $$Count$$ exceeds $$n$$!

For that particular loop, the total number of iterations are

* $$O(k)$$ - in order to iterate over the $$k$$ elements of $$Count$$
* $$O(n)$$ - in order to print out $$n$$ elements to the standard output

This leads to an $$O(n+k)$$ run-time.

### Time Complexity of unrestricted $$k$$
The algorithm runs fast as long as $$k$$ is limited,
...
have $$\Omega{(n \log{n})}$$[^2] lower-bound run-time.
